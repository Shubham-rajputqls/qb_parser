...
import json
file = open("mathematics chapter 6 to  9.html", "r")
fp = file.read()
...
for eachEx in range(0,total_Exercise):
    ...
    def extract_figure_tags(html):
        soup = BeautifulSoup(html, 'html.parser')
        figure_tags = soup.find_all('figure')
        ques_list[len(ques_list) - 1][0].append(figure_tags[0].text)



    def divInsert_withImage(DivTag):
       splitDiv = str(DivTag).split("<br/>")
       extract_figure_tags(splitDiv[0])
       for each in splitDiv:
           br = soup.new_tag('br')
        ...
        if h2_sibling.name == "ol":
            ...
            while True:
                ...
                if (k.name == 'div' and k.get_text().startswith("(")):
                    ...
                    r = k
                    count += 1
                elif ((k.find('img')).name)=='img':
                    divInsert_withImage(k)
                    r = k
                    break
                    ...
                    break
            # while True:
            #     nextImgDiv = r.find_next_sibling()
            #     if nextImgDiv.find('img'):
            #         check = True
            #     else:
            #         check = False
            #
...
out_File = open("MathematicsQuetions3.json", "w")
