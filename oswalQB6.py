...
def extract_this_section(next_all_tags):
    ...
    for each_Que_ans in next_all_tags:
        ...
        Sub_Dict={}
        Sub_Dict["Question-divs"] = Question_list
        Sub_Dict["Answer-divs"] = Answer_list
        Sub_Dict["explaination"] = ""
        Total_Que_list.append(Sub_Dict)
...
def extract_obj_question(next_all_tags):
    ...
    for each_Que_ans in next_all_tags:
        ...
        MCQ_Dict["Answer"] = Answer_list[:1]
        MCQ_Dict["Explaination"] = Answer_list[1:]
        Obj_Que_list.append(MCQ_Dict)
...
def extract_AR_question(next_all_tags):
    ...
    for each_Que_ans in next_all_tags:
        ...
        # Explaination.append(Answer_list[1:])
        AQ_Dict["Assertion"] = Assertion
        AQ_Dict["Reason"] = Reason
        AQ_Dict["Answer"] = Answer_list[:1]
        AQ_Dict["Explaination"] = Answer_list[1:]
        Total_Que_list.append(AQ_Dict)
    return Total_Que_list
...
for each_block in class_blocks:
    ...
    while next_sibling and 'start-heading' not in next_sibling.get('class', []):
        ...
        if next_sibling.name == "h2":
            break
    Main_Dict[each_block.text.replace("\n","")] = extract_this_section(Question_class)
...
for each_block in Obj_class_blocks:
    ...
    while next_sibling and 'start-AR-heading' not in next_sibling.get('class', []):
        ...
        next_sibling = next_sibling.find_next_sibling()
    Main_Dict[re.sub(r'\n?\d+\.\s', '', each_block.text)]=extract_obj_question(Obj_Question_class)
...
for each_block in Obj_class_blocks:
    ...
    while next_sibling and 'start-AR-heading' not in next_sibling.get('class', []):
        ...
        next_sibling = next_sibling.find_next_sibling()
    Main_Dict[re.sub(r'\n?\d+\.\s', '', each_block.text)]=extract_AR_question(AR_Question_class)
...
def convert_to_serializable(obj):
    if isinstance(obj, str):
        ...
        return str(obj)
# Serialize the dictionary values
