...
for each in text_to_delete:
    each.extract()
# print(soup.prettify())
def extract_this_section(next_all_tags,Meta_Dict):
    for each_Que_ans in next_all_tags:
        ...
        while find_next is not None:
            if "start-answer" in find_next.get('class', []):
                ...
                break
            elif "start-topic" in find_next.get('class', []):
                Topic_wise_Dict["topic"] = find_next.text[2:]
            if is_answer_mode:
...
def extract_obj_question(next_all_tags, Obj_Meta_Dict):
    for each_Que_ans in next_all_tags:
        Question_list = []
        Answer_list = []
...
def extract_AR_question(next_all_tags, AR_Meta_Dict):
    for each_Que_ans in next_all_tags:
        Question_list = []
        Answer_list = []
        Question_list.append(each_Que_ans)
...
for each_block in class_blocks:
    ...
    extract_this_section(Question_class,Meta_Dict)
...
for each_block in Obj_class_blocks:
    ...
    extract_obj_question(Obj_Question_class,Obj_Meta_Dict)
...
for each_block in Obj_class_blocks:
    ...
    extract_AR_question(AR_Question_class, AR_Meta_Dict)
